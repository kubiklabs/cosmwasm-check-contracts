...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::ResetAny { count } => try_reset_any(deps, env, info, count),
    }
}
...
fn try_reset(
    ...
    // only allows owner to reset value
...
fn try_reset_any(
    ...
    // Insecure: allows anyone to reset value
    // A static check should identify if contract
    // dev has specified which vaules are controlled
    // or which methods are controlled by admins or a set of users
    constant.count = count;
    CONSTANTS.save(deps.storage, & constant)?;
    Ok(Response::new()
        .add_attribute("action", "COUNT reset successfully"))
